...
"I like basketball."
"I like soccer."
Question: Who likes basketball? Answer:"""  # Alice
...
"I like soccer."
"I like basketball."
Question: Who likes basketball? Answer:"""  # Bob (swapped)
...
print(f"Source (Alice=basketball): {source_pred!r}")
...
print(f"Base (Bob=basketball):     {base_pred!r}")
...
print(f"Source: P(Alice)={source_probs[token_ids['Alice']].item():.4f}, P(Bob)={source_probs[token_ids['Bob']].item():.4f}")
print(f"Base:   P(Alice)={base_probs[token_ids['Alice']].item():.4f}, P(Bob)={base_probs[token_ids['Bob']].item():.4f}")

# Check tracking behavior
print("\n" + "=" * 60)
if source_pred_id == token_ids["Alice"] and base_pred_id == token_ids["Bob"]:
    print("✓ Model correctly tracks conversation!")
elif source_pred_id == token_ids["Alice"] and base_pred_id == token_ids["Alice"]:
    print("✗ Model uses SHORTCUT: Always says Alice")
else:
...
alice_token_id = token_ids["Alice"]
...
for layer_index in trange(model.config.num_hidden_layers, desc="Patching"):
    ...
    for token_index in range(first_diff_index,len(source_prompt_ids)):
        with torch.no_grad():
            ...
            alice_prob = patched_probs[0, alice_token_id].item()
            bob_prob = patched_probs[0, bob_token_id].item()
            patching_per_layer.append(alice_prob - bob_prob)
...
token_strings = [model.tokenizer.decode(tok_id) for tok_id in source_prompt_ids][first_diff_index:]

# Find question position
question_token_id = model.tokenizer('Question').input_ids[1]
question_pos = rindex(source_prompt_ids[first_diff_index:], question_token_id) + first_diff_index

# Plot
plt.figure(figsize=(10, 8))
plt.imshow(patching_results, aspect='auto', cmap='RdBu_r', origin='lower', vmin=-0.5, vmax=0.5)
plt.colorbar(label='P(Alice) - P(Bob)')
...
plt.xlabel('Tokens')
plt.ylabel('Layers')
plt.title('Activation Patching: Source (Alice=basketball) → Base (Bob=basketball)\nOrange = first diff | Green = question | Red = more Alice, Blue = more Bob')
plt.legend(loc='upper left')

plt.tight_layout()
plt.savefig("activation_patching_binding_test.png", dpi=150)
plt.close()

print("\nSaved: activation_patching_binding_test.png")