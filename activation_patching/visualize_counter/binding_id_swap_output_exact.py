...
# ── Fixed example ──────────────────────────────────────────────────────────────
E0, E1, A0, A1 = "Alice", "Bob", "Thailand", "France"
...
fig, axes = plt.subplots(len(CONDS), 1, figsize=(14, 5 * len(CONDS) + 1))
...
for ax, (label, prompt, correct, wrong) in zip(axes, CONDS):
    ...
    for ci, L in enumerate(STEER_LAYERS):
        for ri, alpha in enumerate(ALPHAS):
            ...
            ax.add_patch(rect)
            # Token text (large, centered)
            ax.text(ci + 0.5, ri + 0.62, f'"{top}"',
                    ha="center", va="center", fontsize=8.5,
                    fontfamily="monospace", fontweight="bold")
            # P(correct)/P(wrong) subscript
            ax.text(ci + 0.5, ri + 0.28, f"P(✓)={pc:.2f}  P(✗)={pw:.2f}",
                    ha="center", va="center", fontsize=5.5, color="#444444")

    # Column headers (layers)
    for ci, L in enumerate(STEER_LAYERS):
        ax.text(ci + 0.5, n_rows + 0.15, f"Layer {L}",
                ha="center", va="bottom", fontsize=9, fontweight="bold")

    # Row labels (alphas)
    for ri, alpha in enumerate(ALPHAS):
        ax.text(-0.15, ri + 0.5, f"α={alpha}",
                ha="right", va="center", fontsize=9)

    # Condition title
    cond_short = label.split("\n")[0]
    ax.set_title(cond_short, fontsize=11, fontweight="bold", pad=4)

    # Prompt box below grid
    prompt_display = prompt.replace('\n', '↵  ')
    ax.text(0.5, -0.18, f"Prompt: {prompt_display}",
...
# Legend
legend_patches = [
    mpatches.Patch(color=OUTCOME_COLOR["correct"], label=f'Output = correct country ({A0} or {A1})'),
    mpatches.Patch(color=OUTCOME_COLOR["wrong"],   label=f'Output = wrong country ({A1} or {A0})'),
    mpatches.Patch(color=OUTCOME_COLOR["other"],   label='Output = other token'),
]
fig.legend(handles=legend_patches, loc="upper center", ncol=3,
           fontsize=9, bbox_to_anchor=(0.5, 1.01))

fig.suptitle(
    f"Exact model output under SWAP intervention  ({cfg['name']})\n"
    f"Example: e0={E0}, e1={E1}, a0={A0}(△ in base/hi-confused), a1={A1}\n"
    "SWAP = push △-country → □ and □-country → △ at hook_resid_post of each layer.",
    fontsize=10, y=1.04)

plt.tight_layout(h_pad=3.0)
out = os.path.join(OUTDIR, "binding_id_swap_output_exact.png")
