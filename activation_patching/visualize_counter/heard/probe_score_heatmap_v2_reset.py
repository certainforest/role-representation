"""
probe_score_heatmap_v2_reset.py — Side-by-side binding-ID probe score heatmaps
for two prompts: normal ("Hi Bob!") vs confused ("Hi Alice!").

Layout: heat_n | shared-transcript | heat_c | cbar

The shared transcript shows the diff:
  • "Hi Bob!" → "Hi Alice!"
  • Thailand: ~~blue~~ → red  (confused model binds to Bob; correct is Alice)
  • France:   ~~red~~  → blue (confused model binds to Alice; correct is Bob)

Usage:
  python probe_score_heatmap_v2_reset.py --model llama --gpu 0
"""
...
# ── Run both prompts ───────────────────────────────────────────────────────────
columns_n, mat_n = process_prompt(PROMPT,         UTT_STRINGS["normal"],   'Normal ("Hi Bob!")')
columns_c, mat_c = process_prompt(PROMPT_CONFUSED, UTT_STRINGS["confused"], 'Confused ("Hi Alice!")')
...
def draw_shared_label(fig, ax, columns):
    ...
    utt_content = {
        ...
        # utt4: Thailand swap — ~~blue~~ → red  (1 row, 2 lines → clamp)
        4: [
            [
                ...
                (" ",                   BLACK, "normal"),
                ("Thailand",            RED,   "bold"),
                # (". What about you?\"", BLACK, "normal"),
            ],
        ],
        # utt5: France swap — ~~red~~ → blue  (1 row, 1 line)
        5: [[
            ('"I live in ',      BLACK, "normal"),
...
def draw_heatmap(fig, ax, columns, mat, title):
    ...
    # Row highlights for Thailand + France
    for ri, col in enumerate(columns):
        if col["word"] in HIGHLIGHT_WORDS:
...
out_path = os.path.join(OUTDIR, f"probe_score_heatmap_v2_reset_{args.model}{_suffix}.pdf")
