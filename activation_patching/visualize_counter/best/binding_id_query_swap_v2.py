...
_cache = (os.path.join(PARENT_DIR, f"probes_{args.model}_{args.n_probe}_intercept_basehi.joblib")
          if args.basehi else cache_path(PARENT_DIR, args.model, args.n_probe))
...
# ── Experiment ─────────────────────────────────────────────────────────────────
STEER_LAYERS = list(range(8, 14))   # L8–L13
ALPHAS       = [0, 100, 200]
...
# ── Heatmap: frac correct, with w/o breakdown on interesting cells ────────────
from matplotlib.colors import Normalize

FONT_SIZE  = 10
CMAP       = plt.cm.RdBu_r   # 0.0 = Blue, 1.0 = Red
WO_THRESH  = 0.85   # show "w.../o..." annotation when correct < this

n_layers = len(STEER_LAYERS)
n_alphas = len(ALPHAS)
cell_w   = 1.1
cell_h   = 0.52   # taller to fit two-line annotation
ann_fs   = 9
sub_fs   = 7
panel_w  = n_alphas * cell_w
panel_h  = n_layers * cell_h
hgap     = 0.18

fig_w = len(CONDS) * panel_w + (len(CONDS) - 1) * hgap + 1.2
fig_h = panel_h + 0.6

fig, axes = plt.subplots(1, len(CONDS), figsize=(fig_w, fig_h), sharey=True)
fig.subplots_adjust(wspace=hgap / panel_w)

for ax, (cname, *_) in zip(axes, CONDS):
    mat_c = np.full((n_layers, n_alphas), np.nan)
    mat_w = np.full((n_layers, n_alphas), np.nan)
    mat_o = np.full((n_layers, n_alphas), np.nan)
    for ri, L in enumerate(STEER_LAYERS):
        for ai, alpha in enumerate(ALPHAS):
            entries = results[cname][L][alpha]
            if entries:
                mat_c[ri, ai] = np.mean([e[3] == "correct" for e in entries])
                mat_w[ri, ai] = np.mean([e[3] == "wrong"   for e in entries])
                mat_o[ri, ai] = np.mean([e[3] == "other"   for e in entries])

    ax.imshow(mat_c, aspect="auto", cmap=CMAP, vmin=0, vmax=1, origin="upper")
    ...
    for ri in range(n_layers):
        for ai in range(n_alphas):
            ...
            if np.isnan(vc):
                continue
            txt_color = "white" if (vc < 0.3 or vc > 0.7) else "black"
            if vc < WO_THRESH:
                # Two-line: correct on top, w/o below
                ax.text(ai, ri - 0.15, f"{vc:.2f}",
                        ha="center", va="center", fontsize=ann_fs, color=txt_color)
                ax.text(ai, ri + 0.22,
                        f"w{mat_w[ri,ai]:.2f}/o{mat_o[ri,ai]:.2f}",
                        ha="center", va="center", fontsize=sub_fs, color=txt_color)
            else:
                ax.text(ai, ri, f"{vc:.2f}",
                        ha="center", va="center", fontsize=ann_fs, color=txt_color)
...
out = os.path.join(OUTDIR, f"binding_id_query_swap_v2_{args.model}{_suffix}.pdf")
