...
for k, si in enumerate(IDX[:args.n_probe]):
    ...
    try:
        ...
        # a0 speaker = lbl, a1 speaker = 1-lbl (responds to "What about you?")
        for c, t, lbl in [(cA,tA,1),(cB,tB,0),(cC,tC,0),(cD,tD,1)]:
            X_I.append(extract(c, nth_I(t, 2)));        y_I.append(lbl)
            p0 = first_of(t, a0)
            p1 = first_of(t, a1)
            if p0: X_cty.append(extract(c, p0));        y_cty.append(lbl)
            if p1: X_cty.append(extract(c, p1));        y_cty.append(1 - lbl)
    except Exception:
        continue
    if (k+1) % 30 == 0:
...
fig.suptitle(
    ...
    fontsize=10)
plt.tight_layout()
out = os.path.join(OUTDIR, "binding_id_summary_new.png")
